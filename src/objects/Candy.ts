import { CandyData, Pixel } from '../types/interfaces';
...
export class Candy {
    ...
    render(ctx: CanvasRenderingContext2D): void {
        ...
        // Pulsing glow effect - draw glow around each candy pixel
        const pulse = Math.sin(this.pulseTimer) * 0.3 + 0.7;
        ctx.globalAlpha = pulse * 0.15; // More subtle glow
        ctx.fillStyle = '#FFFFFF'; // White glow for candy
        
        // Draw glow for each candy pixel
        for (const pixel of this.pixels) {
            // Only draw glow for actual candy pixels (not transparent ones)
